...
public class DateUtils {
    ...
    public static final String YMDHMS_FORMATTER = "yyyyMMddHHmmss";
     ...
     * @return
     */
    public static String getDateToStringYMDHMS(Date date) {
        return new DateTime(date).toString(YMDHMS_FORMATTER);
    }
    ...
    public static String getDateToStringYMD(Date date) {
        return new DateTime(date).toString(DATE_DEFAULT_FORMAT_NO_SPACE);
    }
     ...
     * @return 比较两日期大小。如果date==otherDate则返回true,否则false
     */
    public static Boolean isDateEqual(String date, String otherDate) {
        date = date.replace(" ", "T");//把空格替换成T
        otherDate = otherDate.replace(" ", "T");//把空格替换成T
        return isDateEqual(getDate(date), getDate(otherDate));
    }
     ...
     * @return 比较两日期大小。如果date==otherDate则返回true,否则false
     */
    public static Boolean isDateEqual(Date date, Date otherDate) {
        Boolean flag = false;
        Date dateTmp = getDateTime(date);
        Date otherDateTmp = getDateTime(otherDate);
        if (dateTmp != null && otherDateTmp != null) {
    ...
    public static Date setDayPlus(String date, int dayAmount) {
        ...
        DateTime dateTime = new DateTime(date);
        if (dayAmount >= 29) {
            Date[] dates = computeMonthFirstAndLastDay(getDate(date), 0);
            String dateStr = getDateToString(dates[1]);
    ...
    public static List<Date> computeDateRangeDate(Date from, Date to) {
        ...
        from = getDateFormat(from, DATE_DEFAULT_FORMAT);
        to = getDateFormat(to, DATE_DEFAULT_FORMAT);
    ...
    public static Date[] computeMonthFirstAndLastDay(Date sourceDate, int monthInterval) {
        ...
        destDates[0] = getDate(DATE_FORMATTER.get().format(cal.getTime()));
        if (monthInterval < 0) {
            cal.setTime(sourceDate);
            if (monthInterval < -1) {
        ...
        destDates[1] = getDate(DATE_FORMATTER.get().format(cal.getTime()));
...
//        int totalMonth = diff.get(0)*12 + diff.get(1) + (diff.get(2) > 0?1:0);
        ...
        for (int i = 0; i < totalMonth; i++) {
            sourceDate = DateUtils.getDateToString(DateUtils.addMonth(sourceDate, 1));
            String computeDate = sourceDate.substring(0, sourceDate.lastIndexOf("-") + 1);
            Date[] dates = computeMonthFirstAndLastDay(getDate(sourceDate), 0);
            String dateStr = getDateToString(dates[1]);
            String dayStr = dateStr.substring(dateStr.length() - 2);
            Integer day = Integer.valueOf(dayStr);
            if (dayOfMonth <= day) {
                computeDate = computeDate + dayOfMonth + " " + hms;
                Date targetDate = getDate(computeDate);
                dateList.add(targetDate);
            }
        }
        return dateList;
     ...
     * @return
     */
    public static Date getAnyTime(Date date, int hour, int minute, int second) {
        Calendar calendar = Calendar.getInstance();
        if (date == null) {
    ...
    /**
     * String  类型yyyy-MM-dd HH:mm:ss 转成 Date
     *
     * @param str
     * @return
     */
    ...
    public static Date lastSecond(Date date) {
        String endTime = DateFormatUtils.format(date, "yyyy-MM-dd 23:59:59");
        return getDate(endTime);
    }

    public static void main(String[] args) {
//        String am1 = punchInTimeToDateTime("08:30:00");
//        String am2 = punchInTimeToDateTime("12:00:00");
//        String am3 = punchInTimeToDateTime("13:30:00");
//        String am4 = punchInTimeToDateTime("18:30:00");
//        Map<String, List> map = getTimeInterval(am1,am2,am3,am4,"60","30");
//        System.out.println(map);
//        System.out.println(getBeforeDay(getNowDate()));
//        System.out.println(getNowDate());
//        System.out.println(addDateMinute(getNowDate(),30));
//        System.out.println(punchInTimeToDateTime("15:00:00"));
        System.out.println(strToDate("2017-10-03 15:20:00"));
    }
